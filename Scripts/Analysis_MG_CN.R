...
#Read in the table
data <- read.csv("Combined_MG_CN_Dataset.csv")
head(data)
data <- as_tibble(data)
data <- data  %>%
  dplyr::select(1:39)
...
design1<-data.frame(experiment=colnames(rawcounts[,2:39]),  treatment = c("MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN"))
head(design1)

#Add the rownames to the design
rownames(design1)<-colnames(rawcounts[,2:39])
head(design1)

day <- DESeqDataSetFromMatrix(countData = (round(rawcounts[,2:39])), 
...
dday_res <- results(dday,contrast=c("treatment","MG", "CN"))
head(dday_res)
dim(dday_res)
...
volcano_plot <- ggplot(data=dday_res, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label=delabel)) +
  ...
  geom_hline(yintercept=-log10(0.00000000005), col="red") +
  ggtitle(paste('Volcano Plot:', "MG", 'vs.', "CN"))

#Save Plot as a PNG file
ggsave("Volc_CN_MG.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
  ...
  if (is.null(gprofiler_results) || nrow(gprofiler_results$result) == 0) {
    ...
    if (nrow(top_terms) > 0) {
      enrich <- ggplot(top_terms, aes(x = reorder(term_name, -p_value), y = -log10(p_value))) +
        ...
        coord_flip() +
        labs(title = "Enrichment Analysis of MG vs CN",
             x = "Term", y = "-log10(P-value)") +
        theme_minimal()
      #Save Plot as a PNG file
      ggsave("Enrich_MG_CN.png", plot = enrich, width = 8, height = 6, dpi = 300)
    } else {
...
if (nrow(significant_genes_table) > 0) {
  kable(significant_genes_table, format = "html", caption = "Top 10 Significant Genes") %>%
...
pheatmap(sampleDistMatrix,
         ...
         col = colors,
         main = "Heatmap of MG and CNs",
         filename = "HM_MG_CN.png")

#Plot PCA
pca <- plotPCA(vsd, intgroup=c("treatment")) +
  ggtitle("PCA_MG_vs_CN")

ggsave("PCA_MG_CN.png", plot = pca, width = 8, height = 6, dpi = 300)
