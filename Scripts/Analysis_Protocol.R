...
#read in the table
data <- read.csv("Combined_Proto_Dataset.csv")
head(data)
data <- as_tibble(data)
data <- data  %>%
  dplyr::select(1:18)
...
design1<-data.frame(experiment=colnames(rawcounts[,2:18]),  treatment = c("Ramadoss_iN_D3", "Ramadoss_iN_D3", "Ramadoss_iN_D3", "Tian_iN_D14", "Tian_iN_D14", "Lisowski_iN_D14", "Lisowski_iN_D14", "Lisowski_iN_D14", "Lisowski_iN_D14", "Lisowski_iN_D14", "Lisowski_iN_D14", "Susco_iN_D14", "Susco_iN_D14", "Susco_iN_D14", "Susco_iN_D14", "Susco_iN_D14", "Rummel_iN_D30"))
head(design1)
# add the rownames to the design
rownames(design1)<-colnames(rawcounts[,2:18])
head(design1)

day <- DESeqDataSetFromMatrix(countData = (round(rawcounts[,2:18])), 
...
dday_res <- results(dday,contrast=c("treatment","Lisowski_iN_D14", "Susco_iN_D14"))
head(dday_res)
dim(dday_res)
...
volcano_plot <- ggplot(data=dday_res, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label=delabel)) +
  ...
  geom_hline(yintercept=-log10(0.05), col="red") +
  ggtitle(paste('Volcano Plot:', "Lisowski_iN_D14", 'vs.', "Susco_iN_D14"))

#Save Plot as a PNG file
ggsave("Volc_Proto.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
  ...
  if (is.null(gprofiler_results) || nrow(gprofiler_results$result) == 0) {
    ...
    if (nrow(top_terms) > 0) {
      enrich <- ggplot(top_terms, aes(x = reorder(term_name, -p_value), y = -log10(p_value))) +
        ...
        coord_flip() +
        labs(title = "Enrichment Analysis of Lisowski_D14 vs Susco_D14",
             x = "Term", y = "-log10(P-value)") +
        theme_minimal()
      #Save Plot as a PNG file
      ggsave("Enrich_Proto.png", plot = enrich, width = 8, height = 6, dpi = 300)
    } else {
...
if (nrow(significant_genes_table) > 0) {
  kable(significant_genes_table, format = "html", caption = "Significant Genes between Day14 and Day30") %>%
...
selected_data <- long_normalized_counts %>%
  filter(Gene_Name %in% selected_genes & treatment %in% c("Ramadoss_iN_D3", "Tian_iN_D14", "Lisowski_iN_D14", "Susco_iN_D14", "Rummel_iN_D30"))

#Edit colors for graph
custom_colors <- c("Ramadoss_iN_D3" = "#0072B2", "Tian_iN_D14" = "#D95F02", "Lisowski_iN_D14" = "#999999", "Susco_iN_D14" = "#1B9E77", "Rummel_iN_D30" = "#E7298A")
...
Proto <- ggplot(summary_data, aes(x = Gene_Name, y = Mean_Expression, fill = treatment)) +
...
#Save Plot as a PNG file
ggsave("Bar_Proto.png", plot = Proto, width = 8, height = 6, dpi = 300)
...
pheatmap(sampleDistMatrix,
         ...
         col = colors,
         main = "Heatmap of Protocols",
         filename = "HM_Proto.png")

#Plot PCA
pca <- plotPCA(vsd, intgroup=c("treatment")) +
  ggtitle("PCA_Protocols")

ggsave("PCA_Proto.png", plot = pca, width = 8, height = 6, dpi = 300)
