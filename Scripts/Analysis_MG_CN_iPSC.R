...
packages = c("DESeq2", "readxl", "dplyr", "utils", "ggplot2", "R.utils", "gplots", "ggrepel", "gprofiler2", "knitr", "kableExtra", "tidyr")
...
#Read in the table
data <- read.csv("Combined_MG_CN_iPSC_Dataset.csv")
head(data)
data <- as_tibble(data)
data <- data  %>%
  dplyr::select(1:50)
...
design1<-data.frame(experiment=colnames(rawcounts[,2:50]),  treatment = c("MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "MG", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "CN", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC", "iPSC"))
head(design1)

#Add the rownames to the design
rownames(design1)<-colnames(rawcounts[,2:50])
head(design1)

day <- DESeqDataSetFromMatrix(countData = (round(rawcounts[,2:50])), 
...
dday_res <- results(dday,contrast=c("treatment","iPSC", "MG"))
head(dday_res)
dim(dday_res)
...
volcano_plot <- ggplot(data=dday_res, aes(x=log2FoldChange, y=-log10(pvalue), col=diffexpressed, label=delabel)) +
  ...
  geom_hline(yintercept=-log10(0.00000000005), col="red") +
  ggtitle(paste('Volcano Plot:', "iPSC", 'vs.', "MG"))

#Save Plot as a PNG file
ggsave("Volc_iPSC_MG.png", plot = volcano_plot, width = 8, height = 6, dpi = 300)
  ...
  if (is.null(gprofiler_results) || nrow(gprofiler_results$result) == 0) {
    ...
    if (nrow(top_terms) > 0) {
      enrich <- ggplot(top_terms, aes(x = reorder(term_name, -p_value), y = -log10(p_value))) +
        ...
        coord_flip() +
        labs(title = "Enrichment Analysis of iPSC vs MG",
             x = "Term", y = "-log10(P-value)") +
        theme_minimal()
      #Save Plot as a PNG file
      ggsave("Enrich_iPSC_MG.png", plot = enrich, width = 8, height = 6, dpi = 300)
    } else {
...
if (nrow(significant_genes_table) > 0) {
  kable(significant_genes_table, format = "html", caption = "Significant Genes for MG") %>%
...
selected_genes <- c("P2RY12", "GPR34", "CABLES1", "BHLHE41", "TREM2", "OLFML3", "PROS1", "APOE", "SLCO2B1", "SLC7AB", "PPARD", "CRYBB1")
selected_data <- long_normalized_counts %>%
  filter(Gene_Name %in% selected_genes & treatment %in% c("iPSC", "CN", "MG"))

#Edit colors for graph
custom_colors <- c("iPSC" = "#0072B2", "CN" = "#D95F02", "MG" = "#999999")
...
MG <- ggplot(summary_data, aes(x = Gene_Name, y = Mean_Expression, fill = treatment)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = Mean_Expression - SD, ymax = Mean_Expression + SD),
                position = position_dodge(0.9), width = 0.25) +
  labs(title = "Relative Expression of Microglia Genes",
       x = "Gene",
...
#Save Plot as a PNG file
ggsave("Bar_MG.png", plot = MG, width = 8, height = 6, dpi = 300)
...
#Filter for selected genes and the desired treatment groups
selected_genes <- c("NEUROG1", "NEUROG2", "MAP2", "TUBB3")
selected_data <- long_normalized_counts %>%
  filter(Gene_Name %in% selected_genes & treatment %in% c("iPSC", "CN", "MG"))
...
#Create the bar plot with error bars
CN <- ggplot(summary_data, aes(x = Gene_Name, y = Mean_Expression, fill = treatment)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = Mean_Expression - SD, ymax = Mean_Expression + SD),
                position = position_dodge(0.9), width = 0.25) +
  labs(title = "Relative Expression of Cortical Neuron Genes",
       x = "Gene",
       y = "Normalized Expression Level") +
  theme_minimal() +
  scale_fill_manual(values = custom_colors) +
...
#Save Plot as a PNG file
ggsave("Bar_CN.png", plot = CN, width = 8, height = 6, dpi = 300)
...
#Filter for selected genes and the desired treatment groups
selected_genes <- c("SOX2", "NANOG", "POU5F1")
selected_data <- long_normalized_counts %>%
  filter(Gene_Name %in% selected_genes & treatment %in% c("iPSC", "CN", "MG"))
...
#Create the bar plot with error bars
iPSC <- ggplot(summary_data, aes(x = Gene_Name, y = Mean_Expression, fill = treatment)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = Mean_Expression - SD, ymax = Mean_Expression + SD),
                position = position_dodge(0.9), width = 0.25) +
  labs(title = "Relative Expression of iPSC Genes",
       x = "Gene",
       y = "Normalized Expression Level") +
  theme_minimal() +
  scale_fill_manual(values = custom_colors) +
...
#Save Plot as a PNG file
ggsave("Bar_iPSC.png", plot = iPSC, width = 8, height = 6, dpi = 300)
...
pheatmap(sampleDistMatrix,
         ...
         col = colors,
         main = "Heatmap of MG, CN, and iPSCs",
         filename = "HM_MG_CN_iPSC.png")

#Plot PCA
pca <- plotPCA(vsd, intgroup=c("treatment")) +
  ggtitle("PCA Cell Type")

ggsave("PCA_MG_CN_iPSC.png", plot = pca, width = 8, height = 6, dpi = 300)
